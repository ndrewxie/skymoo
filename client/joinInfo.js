...
window.trapBoostSelection = undefined;
window.colorSelection = undefined;

window.jitterSmooth = window.config.JITTER_DELAY;
...
let populatePrimarySecondarySelector = (function() {
	...
	let trapBoostSelector = document.getElementById('trapBoost');
	...
	while (trapBoostSelector.firstChild) {
		trapBoostSelector.removeChild(trapBoostSelector.firstChild);
	}
	
	secondarySelector.style.display = (typeof window.primarySelection == 'undefined') ? "none" : "flex";
	trapBoostSelector.style.display = (typeof window.secondarySelection == 'undefined') ? "none" : "flex";
	
	
	for (let j = 0; j < window.config.unifiedItems.length; j++) {
		if (
			(
				(typeof window.config.unifiedItems[j].wtype != 'undefined') ||
				(j == window.config.getTypeByName("Pit Trap")) ||
				(j == window.config.getTypeByName("Boost Pads"))
			) && 
			(window.config.unifiedItems[j].userSelectable != false)
		) {
			let selectorEntry = window.initElement('div', ['primarySecondOption']);
			if ((j == window.primarySelection) || (j == window.secondarySelection) || (j == window.trapBoostSelection)) {
			...
			if (window.config.unifiedItems[j].wtype == 1) {
				if (typeof window.primarySelection == 'undefined') {
				...
				(typeof window.primarySelection != 'undefined') &&
				(typeof window.secondarySelection != 'undefined') &&
				((j == window.config.getTypeByName("Pit Trap")) || (j == window.config.getTypeByName("Boost pads")))
			) {
				if (typeof window.trapBoostSelection == 'undefined') {
					selectorEntry.appendChild(imageHolder);
				}
				trapBoostSelector.appendChild(selectorEntry);
				selectorEntry.addEventListener('click', function() {
					window.trapBoostSelection = j;
					populatePrimarySecondarySelector();
				});
			}
		}
...
window.addEventListener('load', function() {
	...
	document.getElementById("joinGame").addEventListener('click', function() {
		let smoothSelection = document.getElementById('jitter').value;
		if (smoothSelection == "default") {
			window.jitterSmooth = window.config.JITTER_DELAY;
		}
		else {
			window.jitterSmooth = parseInt(smoothSelection);
		}
	});
});