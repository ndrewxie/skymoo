...
function processZombie(p, shouldCalcBot) {
    if (p.isFreeZombie == true) {
        let targetDistance = undefined;
        let targetPlayer = undefined;
        for (let j = 0; j < p.targetPlayers.length; j++) {
            let tentativeTarget = players[p.targetPlayers[j]];
            if (typeof tentativeTarget != 'undefined') {
                let distance = utils.getSquaredDistance(p.loc[0], p.loc[1], tentativeTarget.loc[0], tentativeTarget.loc[1]);
                if (
                    ((targetDistance == undefined) || (distance < targetDistance)) && 
					(!p.isFriendly(tentativeTarget.playerID)) && 
					(!isReserved(tentativeTarget))
                ) {
        ...
        if (typeof targetPlayer != 'undefined') {
            setAttacker(targetPlayer);
            p.toAttack = targetPlayer.playerID;
        }
        else {
            p.toAttack = undefined;
            p.moveState = 0;
	...
	if (typeof p.toAttack != 'undefined') {
		...
		else if (p.isFreeZombie != true) {
            bots[p.botID] = undefined;
...
function healHelper(p, maxHealSpeed) {
	if (typeof p != 'undefined') {
        if (typeof p.lastBotHeal == 'undefined') {
            p.lastBotHeal = 0;
        }
		if ((p.isSuspended != true) && (Date.now() - p.lastBotHeal >= maxHealSpeed) && (p.health < config.MAX_HEALTH)) {
			p.selectItem(3);
			p.handlePress(false);
			p.botHealTimer = undefined;
            p.lastBotHeal = Date.now();
		}
		else if ((p.isSuspended != true) && (p.health < config.MAX_HEALTH)) {
			registerHealTimer(p, maxHealSpeed + 10 - (Date.now() - p.lastBotHeal), maxHealSpeed);
		}
		else {
			p.botHealTimer = undefined;
		}
...
function processBot(p, shouldCalcBot) {
    if (
        (p.health < config.MAX_HEALTH * 0.8) && 
        (typeof p.botHealTimer == 'undefined') && 
        ((p.botType != "zombie") || (p.isFreeZombie == true))
    ) {
		registerHealTimer(
							p, 
	...
	let currTime = (new Date()).getTime();
	if ((typeof p.touchingTraps != 'undefined') && (p.touchingTraps.length > 0)) {
		if (
    ...
    for (let j = 0; j < bots.length; j++) {
        if (typeof bots[j] != 'undefined') {
            if (players[bots[j]] !== undefined) {
                if (
                    (players[bots[j]].isSuspended != true) &&
                    (players[bots[j]].isBot == true)
                ) {
