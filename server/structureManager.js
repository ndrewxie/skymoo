...
function init(i_connections, i_cells, i_structureCells, i_processPlayerCell, i_players, i_structures, i_gameStructureCells, i_autobreakTimers, i_isBox, i_makeProjectile) {
    ...
    processPlayerCell = i_processPlayerCell;
}
    ...
    for (let j = 0; j < strucVec.length; j++) {
        ...
        if (c !== undefined) {
            ...
            if (utils.isCollision(p.loc[0], p.loc[1], c.loc[0], c.loc[1], actualPlayerRadius, c.radius)) {
				// Rewind time for more accurate collisions
