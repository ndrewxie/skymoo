...
function init(t, proj, projCells, makeProj, conn, i_postToWebhook) {
	...
	structureManager.init(connections, cells, structureCells, processPlayerCell, players, structures, gameStructureCells, autobreakTimers, inventoryManager.isBox, makeProj);
    initializeCells();
}

function deferInit(makeTeam, acceptPlayer) {
    let gameMaster = new Player("Sky", 5, 15, config.getTypeByName("Boost Pads"), [0, 0, 0, 0, 0, 0], "Default");
    ...
    for (let j = 0; j < 2; j++) {
        ...
        let testBot = new Player("Phalanx CIWS", 10, 7, config.getTypeByName("Boost Pads"), testBotStats, "Red");
    ...
    for (let j = 0; j < 3; j++) {
        ...
        let testBot2 = new Player("Nuro", 28, 15, config.getTypeByName("Boost Pads"), testBot2Stats, "Default");
        ...
        testBot2.isFreeZombie = true;
        tbgay.convertBot(testBot2, "zombie");
    }
}
...
function Player(name, primary, secondary, trapBoost, stats, color) {
    this.name = name;
    this.dir = 0;
    ...
    this.setChoices(primary, secondary, trapBoost, stats, color, false);
...
Player.prototype.setChoices = function(primary, secondary, trapBoost, stats, color, isResume=true) {
    this.inventoryManager.setChoices(primary, secondary, trapBoost);
	if (isResume == false) {
...
Player.prototype.suspend = function() {
    ...
    if (this.isBot == true) {
		if ((this.botType != "zombie") || (this.isFreeZombie == true)) {
			setTimeout(function() {
				this.resume(this.name, this.inventoryManager.getPrimary(), this.inventoryManager.getSecondary(), config.getTypeByName("Boost Pads"), this.stats, this.color, connections[this.playerID]);
...
Player.prototype.resume = function(name, primary, secondary, trapBoost, stats, color, connection) {
    this.name = name;
    this.trapped = undefined;
    this.lofted = false;
    this.velX = 0;
    ...
    this.setChoices(primary, secondary, trapBoost, stats, color, true);
...
Player.prototype.handleMeleeHit = function(t) {
	...
	let holdingRange = config.unifiedItems[this.holdingType()].range;
    utils.runAround(this, checkMeleeIntersect, holdingRange + config.EDGE_TOLERANCE, holdingRange + config.EDGE_TOLERANCE);
                        ...
                        if (utils.runAroundResult(k, checkPlaceViability, config.EDGE_TOLERANCE, config.EDGE_TOLERANCE, isCraftAddition) != true) {
							...
							if (typeof isCraftAddition.craftValue != 'undefined') {
								if (typeof k.boxInfo != 'undefined') {
									if (
                                        (typeof k.boxInfo.item != 'undefined') && 
                                        (k.boxInfo.item != config.getTypeByName("No Item")) && 
                                        (typeof k.boxInfo.count != 'undefined') && 
                                        (k.boxInfo.count > 0)
                                    ) {
...
function meleeStructureHelper(the_structure, p, playerRewindLoc) {
...
function meleePlayerHelper(the_player, p, playerRewindLoc) {
...
function checkMeleeIntersect(p, tcgX, tcgY) {
    ...
    let playerVec = cells[tcgX][tcgY];
    let structureVec = structureCells[tcgX][tcgY];
    processStructureCell(structureVec, meleeStructureHelper, p, playerRewindLoc);
    processPlayerCell(playerVec, meleePlayerHelper, p, playerRewindLoc);
}

function playerCollCheckHelper(currentPlayer) {
...
function playerCollCheck() {
    iteratePlayers(playerCollCheckHelper);
}

function collCheckHelper(currentPlayer) {
...
function collCheck() {
    iteratePlayers(collCheckHelper);
}
...
function emitPlayerData(thePlayer, p, playerConcat) {
	...
	else if (thePlayer.isSuspended != true) {
...
var playerSend = new utils.CustomArray();
var structureSend = new utils.CustomArray();
...
function sendStageHelper(thePlayer) {
			...
			for (let j = 0; j < thePlayer.spawners.length; j++) {
				if (typeof thePlayer.spawners[j] != 'undefined') {
					if (typeof thePlayer.spawners[j][1] == 'undefined') {
						...
						let testBot = new Player("Zombie", config.getTypeByName("Katana"), 15, config.getTypeByName("Boost Pads"), testBotStats, "White");
						testBot.stats = testBotStats;
						connections[testBot.playerID] = new tbgay.dummyConnection();
						...
						let respawnStructure = structures[thePlayer.spawners[j][0]];
						toRespawn.resume(toRespawn.name, toRespawn.inventoryManager.getPrimary(), toRespawn.inventoryManager.getSecondary(), config.getTypeByName("Boost Pads"), toRespawn.stats, toRespawn.color, connections[toRespawn.playerID]);
	...
	if (connections[j] !== undefined) {
		if (thePlayer.isBot != true) {
			...
			playerSend.push(msgpack.PLAYER_UPDATE_DELIM);
			structureSend.push(msgpack.STRUCTURE_DELIM);
		}
		...
		if (thePlayer.isBot != true) {
			playerSend.finalize();
			structureSend.finalize();
		
			connections[j].send(new Uint8Array(playerSend.bufferData));
			if (structureSend.length > 1) {
				connections[j].send(new Uint8Array(structureSend.bufferData));
			}
...
let numTicks = 0;
let tickResolution = 12;
let longTickTime = Math.min(config.TICK_INTERVAL - 5 * tickResolution, 0.5 * config.TICK_INTERVAL);

function startTicks(forceStart=false) {
	let currTime = Date.now();
    let targetTime = tickEpoch + config.TICK_INTERVAL * numTicks;
	if (forceStart || (targetTime - currTime <= tickResolution)) {
		tick();
		currTime = Date.now();
        numTicks += 1;
        targetTime = tickEpoch + config.TICK_INTERVAL * numTicks;
	}
	
	let delayTime = targetTime - currTime;
	if (delayTime > longTickTime) {
		setTimeout(startTicks, longTickTime);
	}
	else if (delayTime > tickResolution) {
		setTimeout(startTicks, tickResolution);
	}
	else {
		startTicks(true);
	}
