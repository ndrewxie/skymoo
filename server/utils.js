...
// Faster occludes for bots
function fastOccludes(p0, p1, b0, b1, s0, s1, br, sr) {
    // An object can't occlude the view if
    // it's on the wrong side
    if (((b0 + br < p0) && (s0 > p0)) || 
        ((b0 - br > p0) && (s0 < p0))) {
...
CustomArray.prototype.push = function(toInsert) {
	this.bufferData[this.length] = toInsert;
	this.length += 1;
}
CustomArray.prototype.finalize = function() {
	if (this.length < this.lastLength) {
		this.bufferData.length = this.length;
	}
}
...
exports.fastOccludes = fastOccludes;
