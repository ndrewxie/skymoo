...
var server = http.createServer(function(req, res) {
    if ((req.method === 'GET') && req.url.includes('.js')) {
		...
		let fixedInput = req.url.replace(/\W/g, '').toLowerCase();
		for (let j = 0; j < cachedImages.length; j++) {
			if (cachedImages[j][0].toLowerCase() == fixedInput) {
				res.writeHead(200, {'Content-Type': 'image/png'});
                res.write(cachedImages[j][1]);
                return res.end();
			}
		}
		if (typeof foundIndex == 'undefined') {
			fs.readFile('./img' + req.url, function(err, data) {
				if (typeof data !== 'undefined') {
					cachedImages.push([fixedInput, data]);
				...
				if (binData[0] == msgpack.JOIN) {
					if (typeof user === 'undefined') {
						...
						if (typeof respawnData !== 'undefined') {
							...
							let trapBoost = respawnData[3];
							let upgradePoints = respawnData[4];
							let colorName = respawnData[5];
							user.resume(playerName, primaryWeapon, secondaryWeapon, trapBoost, upgradePoints, colorName, connection);
						}
					}
				}
...
function extractJoinData(binData) {
	...
	if (typeof proposedPrimaryWeapon != 'undefined') {
		if (
			(typeof config.unifiedItems[proposedPrimaryWeapon] != 'undefined') && 
			(config.unifiedItems[proposedPrimaryWeapon].wtype == 1) &&
			(config.unifiedItems[proposedPrimaryWeapon].userSelectable != false)
		) {
	...
	if (typeof proposedSecondaryWeapon != 'undefined') {
		if (
			(typeof config.unifiedItems[proposedSecondaryWeapon] != 'undefined') && 
			(config.unifiedItems[proposedSecondaryWeapon].wtype == 2) &&
			(config.unifiedItems[proposedSecondaryWeapon].userSelectable != false)
		) {
	...
	let trapBoost = config.getTypeByName("Pit Trap");
	let proposedTrapBoost = msgpack.expectNumb(data);
	if (typeof proposedTrapBoost != 'undefined') {
		if ((proposedTrapBoost == config.getTypeByName("Pit Trap")) || (proposedTrapBoost == config.getTypeByName("Boost Pads"))) {
			trapBoost = proposedTrapBoost;
		}
	}
	else {
		return undefined;
    ...
    if ((typeof playerName !== 'undefined') && (totalPoints <= config.TOTAL_POINTS) && (upgradePoints.length == config.statPoints.length) && (typeof colorName !== 'undefined')) {
        ...
        return [playerName, primaryWeapon, secondaryWeapon, trapBoost, upgradePoints, colorName];
    }
    return undefined;
}
...
function waitJoin(binData, connection, uniqueHash) {
    ...
    if (typeof joinData !== 'undefined') {
		...
		let trapBoost = joinData[3];
        let upgradePoints = joinData[4];
        let colorName = joinData[5];   
        let a = new serverData.Player(playerName, primaryWeapon, secondaryWeapon, trapBoost, upgradePoints, colorName); //, 7
